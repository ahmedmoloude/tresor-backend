    ...
    public function exportcontribuablePDF($id)
{
    ...
    public function getFilteredContribuables(Request $request)
    {
        ...
        if ($montantMinimum !== null) {
            $query->havingRaw('SUM(CAST(roles_contribuables.montant AS DECIMAL)) - COALESCE(SUM(CAST(roles_contribuables.montant_paye AS DECIMAL)), 0) >= ?', [$montantMinimum]);
        }
